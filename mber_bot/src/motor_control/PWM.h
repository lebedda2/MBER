...
inline int controlPwmFile(PWM_CHAN pwm, char *control_file, const char * value)
{
...
inline int setEnable(PWM_CHAN pwm, bool enable)
{
	char value[128];
	int check;

	if (enable == true)
		check = snprintf(value, 128, "%s", "1");
	else
		check = snprintf(value, 128, "%s", "0");
...
inline int setPeriod(PWM_CHAN pwm, unsigned long period)
{
	char buffer[128];
	int check;
...
inline int setDutyCycle(PWM_CHAN pwm, unsigned long duty_cycle)
{
	char buffer[128];
	int check;
...
inline int setPolarity(PWM_CHAN pwm, POLARITY polarity)
{
	char *value;
